...
int main(int argc, char const *argv[])
{
  if(argc!=2){
    ...
    std::vector<std::string> ztt_sample_name={"mc16a_ztautau_NOMINAL.root","mc16e_ztautau_NOMINAL.root","mc16d_ztautau_NOMINAL.root"};
  ...
  TFile *outputfile=new TFile((std::string(PACKAGE_DIR)+"/FF_SS_"+argv[1]+".root").c_str(),"recreate");
  outputfile->cd();
  outhist1p->Write();
  outputfile->Close();
