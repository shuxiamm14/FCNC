...
std::map<TString,variable*> getVariables(TString framework){
	...
	if(framework == "xTFW"){
		...
		ret["tautauvispt"] = new variable("tautauvispt","P_{t,#tau#tau,vis}",100,50.,250.,"GeV",scale,1);
		ret["t2vismass"] = new variable("t2vismass","m_{t,FCNC,vis}",100,50.,250.,"GeV",scale,1);
		ret["t1vismass"] = new variable("t1vismass","m_{t,SM,vis}",100,50.,250.,"GeV",scale,1);
		ret["x1fit"] = new variable("x1fit","E_{vis-#tau,1}/E_{#tau,1}",70,0.3,1.,"",1,1);
		ret["x2fit"] = new variable("x2fit","E_{vis-#tau,2}/E_{#tau,2}",70,0.3,1.,"",1,1);
		ret["chi2"] = new variable("chi2","#chi^{2}",100,0.,20.,"",1,1);
