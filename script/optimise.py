...
def runTrainBDT(current_cut_tmp,current_tree_tmp):#5,10
    command_cut_direction  ="trainBDT_run " + channel + " 2 "+str(current_cut_tmp+cut_step)+" "+str(current_tree_tmp)+"  |grep optmisationScore"
    command_tree_direction ="trainBDT_run " + channel + " 2 "+str(current_cut_tmp)+" "+str(current_tree_tmp+tree_step)+"  |grep optmisationScore"
...
p = subprocess.Popen("trainBDT_run " + channel + " 2  5  10 |grep optmisationScore", shell=True, stdout=subprocess.PIPE)
