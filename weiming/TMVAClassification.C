...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   //TString fname_s = "../../Hist/25ns_R21SkimV7/FakeMini/MytthAnaSkim_thq_aMCpy8_mc16adv7.root";
   TString fname_s = "../../Hist/25ns_R21SkimV7/FakeMini/MytthAnaSkim_thqsumtautau_aMCpy8_mc16adv7.root";
   TString fname_b = "../../Hist/25ns_R21SkimV6/FakeMini/MytthAnaSkim_ttbarnohad_summed_mc16adv6.root";
   TString fname_db = "../../Hist/25ns_R21SkimV6/FakeMini/MytthAnaSkim_data_combined_25ns_R21_data151617v6.root";

   TFile *input_s = TFile::Open( fname_s );
   TFile *input_b = TFile::Open( fname_b );
   TFile *input_db = TFile::Open( fname_db );

   std::cout << "--- TMVAClassification       : Using input file: " << input_s->GetName() << std::endl;
   std::cout << "--- TMVAClassification       : Using input file: " << input_b->GetName() << std::endl;
   std::cout << "--- TMVAClassification       : Using input file for test: " << input_db->GetName() << std::endl;

   TTree *signal     = (TTree*)input_s->Get("Tmva1l2tau");
   TTree *background = (TTree*)input_b->Get("Tmva1l2tau");
   TTree *background_test = (TTree*)input_db->Get("Tmva1l2tau");
   ...
   //dataloader->AddVariable( "Njets25", 'F' );
   //dataloader->AddVariable( "Htjets", 'F' );
   dataloader->AddVariable( "Mtw", 'F' );
   dataloader->AddVariable( "Lept*Leadtauch>0?Leadtaupt:Subtaupt", 'F' );
   dataloader->AddVariable( "Lept*Leadtauch<0?Leadtaupt:Subtaupt", 'F' );
   //dataloader->AddVariable( "Leadtaupt", 'F' );
   //dataloader->AddVariable( "Subtaupt", 'F' );
   ...
   TCut cutTest = "Event>0";
   TCut cutTrain = "Event>0";
   //dataloader->AddTree(signal, "Signal", signalWeight, cutTrain, "train");
   //dataloader->AddTree(signal, "Signal", signalWeight, cutTest, "test");
   dataloader->AddSignalTree    ( signal,     signalWeight     );
   dataloader->AddBackgroundTree( background, backgroundWeight );
   //dataloader->AddBackgroundTree( background, backgroundWeight, "Training");
   //dataloader->AddBackgroundTree( background_test, backgroundWeight, "Testing");
   ...
   TCut mycuts = "Evtclass==2&&Leptrig&&Njets25>1&&Nbjets25==1&&(Leadtautruth==10&&Subtautruth==10)&&(Leadtauch*Subtauch<0)&&(Leadtautight+Subtautight)>=0&&Leadtaubtag<0.83&&Subtaubtag<0.83&&Weight>0.";
   TCut mycutb = "Evtclass==2&&Leptrig&&Njets25>1&&Nbjets25==1&&(Leadtauch*Subtauch<0)&&(Leadtautight+Subtautight)>=0&&Leadtaubtag<0.83&&Subtaubtag<0.83&&(Weight==1?(Leadtauch*Subtauch>0):Weight>0)";
